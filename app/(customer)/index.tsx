      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.balanceCard}>
          <View style={styles.balanceHeader}>
            <View>
              ...
              <Text style={styles.balanceValue}>{demoCustomer.points.toLocaleString()}</Text>
            </View>
          ...
          {demoCustomer.nextTier && (
            <View style={styles.progressSection}>
              <ProgressBar current={demoCustomer.points} max={tierMax} color={colors.white} />
              <Text style={styles.progressText}>{demoCustomer.pointsToNextTier.toLocaleString()} points to {demoCustomer.nextTier}</Text>
            </View>
          )}
        </View>
        ...
        <View style={styles.quickActions}>
          {QUICK_ACTIONS.map((action) => (
            <TouchableOpacity key={action.label} style={styles.quickActionItem} onPress={() => router.push(action.route as any)} activeOpacity={0.7}>
              <View style={styles.quickActionCircle}><Ionicons name={action.icon} size={24} color={colors.primary} /></View>
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <TouchableOpacity onPress={() => router.push('/(customer)/companies')}><Text style={styles.seeAll}>See All</Text></TouchableOpacity>
          </View>
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <TouchableOpacity onPress={() => router.push('/(customer)/activity')}><Text style={styles.seeAll}>See All</Text></TouchableOpacity>
          </View>
          <Card padding="none">
...
const styles = StyleSheet.create({
  ...
  greeting: { ...typography.h2, color: colors.charcoal, marginTop: spacing.md, marginBottom: spacing.lg },
  balanceCard: { backgroundColor: colors.primary, borderRadius: borderRadius.xl, padding: spacing.lg, marginBottom: spacing.lg },
  balanceHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' },
  balanceLabel: { ...typography.caption, color: 'rgba(255,255,255,0.8)', marginBottom: spacing.xs },
  balanceValue: { fontSize: 36, fontWeight: '700', color: colors.white, lineHeight: 42 },
